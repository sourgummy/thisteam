...
public class NoticeWriteProService {
		...
		// 글쓰기 작업 요청을 위한 registNotice() 메서드 정의
		public boolean registNotice(NoticeBean notice) {
			...
			// 3. NoticeDAO 클래스로부터 NoticeDAO 객체 가져오기(공통)
			// => 싱글톤 디자인 패턴으로 구현되어 있는 객체를 getInstance() 메서드로 리턴받기
			NoticeDAO dao = NoticeDAO.getInstance();
			
			// 4. NoticeDAO 객체의 setConnection() 메서드를 호출하여 Connection 객체 전달(공통)
			dao.setConnection(con);
						
			// 5. NoticeDAO 객체의 xxx() 메서드를 호출하여 xxx 작업 수행 요청 및 결과 리턴받기
			//    insertNotice() 메서드를 호출하여 글쓰기 작업 요청 및 결과 리턴받기
			// => 파라미터 : NoticeBean 객체   리턴타입 : int(insertCount)
