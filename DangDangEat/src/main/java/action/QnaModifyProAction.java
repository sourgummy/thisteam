...
//			System.out.println("원본 파일명 : " + review.getQna_file());
//			System.out.println("실제 파일명 : " + review.getQna_real_file());
			
			// QnaModifyProService - isQnaWriter() 호출하여 패스워드 일치 여부 확인
			// => 파라미터 : QnaBean 객체    리턴타입 : boolean(isQnaWriter)
			...
			if(!isQnaWriter) { // 수정 권한 없음
				...
				// QnaModifyProService - modifyQna() 호출하여 글 수정 작업 요청
				...
				// 성공 시 ActionForward 객체를 통해 "QnaDetail.bo" 페이지로 포워딩(Redirect)
