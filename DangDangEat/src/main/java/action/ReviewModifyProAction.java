...
//			System.out.println("실제 업로드 경로 : " + realPath);
			...
			review.setReview_code(Integer.parseInt(multi.getParameter("review_code")));
			review.setMember_id(multi.getParameter("member_id"));
			review.setReview_pass(multi.getParameter("review_pass"));
			review.setReview_subject(multi.getParameter("reivew_subject"));
...
//			System.out.println("실제 파일명 : " + board.getBoard_real_file());
			...
			if(!isReviewWriter) { // 수정 권한 없음
				...
				if(!isModifySuccess) { // 수정 실패 시 
					...
					forward.setPath("ReviewDetail.bo?review_code=" + review.getReview_code() + "&pageNum=" + multi.getParameter("pageNum"));
					...
					if(review.getReview_file() != null) {
						deleteFileName = multi.getParameter("review_real_file");
					}
				}
			}
		} catch (IOException e) {
